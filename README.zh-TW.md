...
> 本文件譯自 v0.9.0 之後的英文 README（2026-07-19 同步），採用台灣慣用詞彙。
...
**v0.9.0**；完整歷史見 [CHANGELOG.md](CHANGELOG.md)，互動式藍圖見
[藍圖討論](https://github.com/wilsonglasser/oryxis/discussions/67)。
正在推進的方向包括：進階驗證（SSH 憑證、FIDO2 安全金鑰、PKCS#11）、
終端機強化（回捲搜尋、分割窗格廣播輸入、命令選擇區）、原生 Mosh
用戶端、設定匯入工具（PuTTY / WinSCP / mRemoteNG / Termius）、主機
監控面板，以及東亞全形寬度選項和終端機背景圖。
