...
fn hotkey_category(action: HotkeyAction) -> PaletteCategory {
    ...
    match action {
        ...
        | OpenSftp => PaletteCategory::Tabs,
        OpenPortForwards | FocusViewSearch | NewHost | NewKey | NewIdentity
    ...
    pub(crate) fn palette_rows(&self, query: &str) -> Vec<PaletteRow> {
        ...
        for &action in HotkeyAction::all() {
            ...
            candidates.push(Candidate {
                ...
                message: Message::RunHotkeyAction(action),
                enabled,
            });
        }
        ...
        for (label, section) in self.settings_section_items() {
            candidates.push(Candidate {
                ...
                message: Message::OpenSettingsSection(section),
                enabled: true,
            });
        }
