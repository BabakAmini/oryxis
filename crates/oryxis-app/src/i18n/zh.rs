...
pub(super) fn lookup(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "biometric_unlock_setting" => "生物识别解锁",
        "biometric_unlock_desc" => "使用设备的生物识别（Windows Hello、Touch ID）解锁此应用，在 Linux 上则使用登录密钥环。您的主密码存储在操作系统的受保护密钥库中，并在通过在场检查后释放。这仅解锁应用，并非 SSH 身份验证，保险库无论如何都会继续使用您的密码加密。",
        ...
        "tab_bar_position_desc" => "底部会把标签栏停靠在状态栏上方；窗口按钮保留在顶部的细条中。",
