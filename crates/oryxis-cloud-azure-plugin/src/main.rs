//! `oryxis-cloud-azure-plugin`, the Microsoft Azure provider as a
...
//! GCP plugins and `oryxis-mcp` use. The main app spawns this binary, runs
//! the `initialize` handshake, and drives discovery / credentials through
//! it.
//!
//! Like the K8s / GCP plugins this carries no SDK: every operation shells
//! out to `az`. It stays a separate subprocess only for architectural
...
async fn main() {
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("oryxis_cloud_azure_plugin=info".parse().unwrap()),
        )
        .init();

    tracing::info!(
        version = env!("CARGO_PKG_VERSION"),
        "oryxis-cloud-azure-plugin started"
    );
    ...
    tracing::info!("stdin closed, oryxis-cloud-azure-plugin exiting");
}
