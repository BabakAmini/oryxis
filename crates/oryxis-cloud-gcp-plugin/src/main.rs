//! `oryxis-cloud-gcp-plugin`, the Google Cloud provider as a standalone
//! subprocess.
//!
...
//! `gcloud`. It stays a separate subprocess only for architectural
...
async fn main() {
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("oryxis_cloud_gcp_plugin=info".parse().unwrap()),
        )
        .init();

    tracing::info!(
        version = env!("CARGO_PKG_VERSION"),
        "oryxis-cloud-gcp-plugin started"
    );
    ...
    tracing::info!("stdin closed, oryxis-cloud-gcp-plugin exiting");
}
