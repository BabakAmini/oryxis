//! Map JSON-RPC method calls onto `oryxis-cloud-gcp`.
//!
//! GCP Compute implements the discovery surface (credentials, discover,
//! transport-metadata query). There is no `resolve_query` family (no
//! dynamic-group analog) and no transport operation here: Compute VMs are
//! reached over plain SSH from the app, and GKE clusters are served by the
//! Kubernetes provider once a kubeconfig is fetched.
...
mod tests {
    ...
    #[tokio::test]
    async fn initialize_returns_gcp_metadata() {
        let resp = handle(
            method::INITIALIZE,
            Value::from(1),
        ...
        assert_eq!(init.provider_id, "gcp");
