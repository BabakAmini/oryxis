...
창 불투명도), 네이티브 Mosh 클라이언트, 설정 가져오기
도구(PuTTY / WinSCP / mRemoteNG / Termius), 호스트 모니터링의 GPU
게이지와 멀티 호스트 대시보드, 동아시아 모호 폭(ambiguous width)
옵션과 터미널 배경 이미지. 커뮤니티 요청도 예정되어 있습니다:
JumpServer류 배스천 로그인 스크립트, sudo 프롬프트에서 볼트
비밀번호 사용, 사이드바 tmux 세션 관리자, 필요할 때 내려받는
터미널 폰트 팩, 호스트 트리 뷰.
