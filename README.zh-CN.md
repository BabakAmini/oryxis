...
> 本文档基于 v0.9.0 之后的英文 README 翻译（2026-07-11 同步）。
...
**v0.9.0**；完整历史见 [CHANGELOG.md](CHANGELOG.md)，交互式路线图见
[路线图讨论](https://github.com/wilsonglasser/oryxis/discussions/67)。
正在推进的方向包括：高级认证（SSH 证书、FIDO2 安全密钥、PKCS#11）、
终端增强（回滚搜索、分屏广播输入、命令面板）、原生 Mosh 客户端、
配置导入器（PuTTY / WinSCP / mRemoteNG / Termius）、主机监控面板，
以及面向中文用户的阿里云 / 腾讯云支持、东亚宽度选项和终端背景图。
