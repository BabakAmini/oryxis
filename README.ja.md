...
ホスト監視のマルチホストダッシュボード、東アジアの
曖昧幅（ambiguous width）オプションとターミナル背景画像。コミュ
ニティからの要望も予定に入っています：JumpServer 系踏み台のログ
インスクリプト、sudo プロンプトでの保管庫パスワード利用、サイド
バーの tmux セッションマネージャー、オンデマンドで取得できるター
ミナルフォントパック、ホストのツリービュー。
