...
Latest stable: **v0.8.3**. Everything in the **Unreleased** row has
already landed on `main` and ships with the next release; see
[CHANGELOG.md](CHANGELOG.md#unreleased) for the full notes.

| Version | Status | Scope |
|---------|--------|-------|
| **Unreleased** | On `main` | **Ad-hoc quick connect** (`user@host` in the new-tab picker without saving a host, with saved identity / key switching offered right at a failed auth prompt); **per-host command history** (a History tab in the terminal sidebar: most-frequent + recent, run / paste / delete, captured via shell-integration marks and never synced; with an explicit plain-text export and an optional per-host live-append log for offline reference); **session recording export** (the encrypted session logs gain real timing and export as asciicast v3 `.cast` files with the terminal theme embedded, or plain-text transcripts, output-only by design); **snippet variables** (`{placeholder}` fields prompted at run time, with defaults); **paste guard content heuristics** (single-line pastes with homograph characters, pipe-to-shell one-liners or hidden control sequences hit the confirmation too); **per-snippet shortcuts** (record a key combo on a snippet, runs it in the focused terminal); **snippet groups + tags and host tags** (grouped snippet lists, tag search everywhere, a tag filter on the host grid, and a sidebar toggle that surfaces only the snippets tagged like the focused host); **TOTP 2FA in the vault** (encrypted per-host secret, verification-code prompts answered automatically); **vault auto-lock** (idle soft lock that keeps live sessions alive through unlock); **unified keyboard navigation** (focus zones across the vault, modals, Settings, side panels and the whole terminal sidebar: Ctrl+Shift+H cycles the sidebar tabs, Tab walks every control, Enter runs the selected snippet / command, Ctrl+Shift+B toggles the sidebar); **careful paste** (multi-line confirmation with preview + bracketed-paste support); **tab rename** and **bottom tab bar**; **Ctrl+Tab** most-recently-used tab switching; **never-stored password-prompt auth** + legacy RSA hash negotiation; **window-geometry persistence**; **debug logging + environment info** (Settings > Advanced); **renderer auto-probe** (software fallback when the GPU stack is software-rasterized, truthful backend labels, restart-to-apply); **performance mode**; AI chat harness polish (per-tab routing, Plan / Ask / Auto modes, floating Stop, privacy redaction); privacy-mode expansion (IPv6 / user@host / vault hostnames, progress-panel leaks plugged); fixes for pasted CRLF doubling (#60), raw-mode prompt freezes over SSH (#48) and ECS dynamic-group re-import |
| **v0.9** | Planned | Google Cloud provider (Compute Engine + GKE); Azure provider (VMs + AKS); **Telnet protocol** (a per-host protocol selector: SSH stays the default, Telnet is an opt-in for legacy gear that speaks nothing else, network switches / routers, serial-console terminal servers, embedded / industrial devices, with a reduced editor for username / password only and a cleartext note; raw TCP plus RFC 854/855 IAC option negotiation, NAWS window-size and `xterm-256color` terminal-type, no keys / jump / proxy / SFTP / forwards); **serial port** (a third `TerminalTransport` alongside SSH and Telnet, riding the same reduced-editor pattern: device path plus baud / data bits / parity / stop bits, for direct console cables); **RDP / VNC over SSH in one click** (a port-forward rule or host opens the OS-native remote-desktop client through an auto-provisioned `-L` tunnel: `mstsc` on Windows, FreeRDP / Remmina on Linux, Microsoft Remote Desktop on macOS), cross-platform where Bitvise is Windows-only; **smart tabs** (built on the same OSC 133 prompt-marker parser as command history: detect the current foreground process and command start / end, notify when a long-running command finishes or when output resumes after a quiet period, with an optional OSC 9;4 progress bar on the tab); **PuTTY parity pack** (the small things every PuTTY hand expects: middle-click paste with a configurable right-click action, the SSH pre-authentication banner shown instead of silently dropped, TCP_NODELAY on interactive sessions, and a per-host IPv4 / IPv6 preference); biometric unlock; Windows ConPTY local shell; Windows JumpList (recent hosts in the taskbar menu); XChaCha20-Poly1305 wire format (192-bit nonce) on a sync v6 protocol bump |
| **v1.0** | Planned | **Stable 1.0**. Advanced authentication: SSH certificate auth (signed user certs), FIDO2 / security-key keys (`sk-ssh-ed25519`, `sk-ecdsa-sk`), and PKCS#11 / smartcard / YubiKey hardware-backed keys; in-app SSH key generation (Ed25519 / RSA / ECDSA with optional passphrase). Terminal: scrollback search (Ctrl+F find-in-buffer with match highlighting), broadcast / synchronized input across split panes (with independent broadcast channels for distinct server groups, and optional synchronized scrolling), zmodem / lrzsz in-terminal transfer (send / receive files straight from the prompt with `sz` / `rz`, for the in-a-jump-host case where opening SFTP is awkward), **X11 forwarding** (`-X` / `-Y` trusted / untrusted, opening `x11-req` channels to the local display; works on Wayland desktops via Xwayland, native Wayland / `waypipe` is out of scope), OSC 8 hyperlinks, keyword highlighting with triggers (user-defined regex rules that colour matches such as ERROR lines or a production hostname, and can optionally fire an action on match: a notification, a sound, or sending a stored snippet), quick-connect OS integration (the shipped `user@host` quick connect registered as the OS `ssh://` URL handler and exposed as an `oryxis user@host` CLI entrypoint), and a command palette. **GIF export of session recordings** (render a v0.9 `.cast` session recording into a shareable GIF, planned as an optional downloadable plugin so font rendering and GIF encoding stay out of the core binary). **Mosh protocol** (roaming + predictive echo: a native Rust implementation of mosh's State Synchronization Protocol, client side, interoperating with the standard `mosh-server` shipped in every distro; sessions survive IP changes, sleep / resume and flaky links, with mosh's predictive local echo ported as an overlay on the terminal grid. No Rust implementation of the protocol exists today and other GUI clients embed the C++ code instead, so this would also make Oryxis the first mosh client that runs natively on Windows). **Config importers** (PuTTY registry sessions, WinSCP, mRemoteNG and Termius exports, alongside the existing `~/.ssh/config` import); **Wake-on-LAN** (send a magic packet to a sleeping host from its card); **connection health** (a per-tab latency indicator, plus reusing an already-open SSH connection when a second tab to the same host opens); **Files sidebar in the terminal** (a per-session file browser docked next to Chat and Snippets, multiplexed over the live SSH connection, following the shell's working directory as you `cd`, with one click to open the full SFTP manager at that path); **group settings inheritance** (groups carry per-parameter defaults, credentials, proxy, environment variables, theme, startup snippet, that hosts and subgroups inherit unless overridden, Termius-style fleet management); **legacy keyboard modes** (per-host backspace ^H / ^?, rxvt Home / End and function-key styles for the same network-appliance audience as Telnet and serial, plus per-host toggles to disable mouse reporting / remote resizing / title changes). **AI ops toolkit** (the terminal AI assistant graduates from generating shell commands to calling typed, structured operations: service status / restart, log queries, process and disk inspection, file operations, each synthesized for the host's actual OS and init system from a per-host facts model, so no more Ubuntu-flavoured commands on an Alpine box; read-only operations run freely, every state-changing operation shows a human-readable dry-run preview and asks first, and an audit journal records everything the agent did; local-first and bring-your-own-key, no hosted backend, secrets never enter the model's context). Consistency + stability: a unified form system (shared field / footer / inline-error helpers adopted across every editor) and portable export/import test hardening |
